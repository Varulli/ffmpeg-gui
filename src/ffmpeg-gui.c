...
typedef struct ResizeRenderData
{
    SDL_Window *window;
    int windowWidth;
    int windowHeight;
...
Clay_RenderCommandArray createLayout()
{
    ...
    CLAY({.id = CLAY_ID("OuterContainer"), .layout = {.sizing = {CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0)}, .padding = CLAY_PADDING_ALL(16), .childGap = 16}, .backgroundColor = {250, 250, 255, 255}})
    {
              ...
              .backgroundColor = COLOR_LIGHT})
        {
            ...
            {
                CLAY({.id = CLAY_ID("ProfilePicture"), .layout = {.sizing = {.width = CLAY_SIZING_FIXED(60), .height = CLAY_SIZING_FIXED(60)}}, .image = {.imageData = sample_image, .sourceDimensions = {60, 60}}}) {}
...
int main(int argc, char *argv[])
{
    ...
    TTF_Font *font = TTF_OpenFont("resources/ostrich-regular.ttf", 16);
    if (!font)
    {
    ...
    } //"SDL_RENDERER_ACCELERATED" is for antialiasing
    else
    {
